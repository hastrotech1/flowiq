    ...
    >
      ...
      <div className="flex gap-6 h-full">
        ...
        <aside className="hidden lg:flex flex-col w-72 shrink-0">
          <SectionCard noPadding className="sticky top-4 flex flex-col overflow-hidden">
            <SectionHeader
              title="Narrations"
          ...
          <PageWrapper fluid>
            ...
            {hasActiveFilters && (
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs text-data-secondary">Active filters:</span>

                {narrationSearch && (
                  <FilterChip
                    label={`"${narrationSearch}"`}
                    onRemove={() => useFiltersStore.getState().setNarrationSearch('')}
                  />
                )}
                {transactionType !== 'all' && (
                  <FilterChip
                    label={transactionType === 'debit' ? 'Debits only' : 'Credits only'}
                    onRemove={() => useFiltersStore.getState().setTransactionType('all')}
                  />
                )}
        ...
        <>
          ...
          <div className="lg:hidden fixed bottom-0 inset-x-0 z-50 bg-white rounded-t-[20px] shadow-xl max-h-[80dvh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-surface-border shrink-0">
              <div>
                <div className="w-10 h-1 bg-surface-muted rounded-full mx-auto mb-3" aria-hidden="true" />
                <h3 className="text-sm font-semibold text-gray-900">Narrations</h3>
                <p className="text-xs text-data-secondary">Tap to filter by narration</p>
              </div>
              <button
                onClick={() => setShowNarPanel(false)}
